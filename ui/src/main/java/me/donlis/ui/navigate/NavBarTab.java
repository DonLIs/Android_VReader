...
import me.donlis.ui.R;

public class NavBarTab extends FrameLayout {

    private int unselect_color = Color.parseColor("#c9c9c9");

    private int primary_color = Color.parseColor("#F44336");
    ...
    public NavBarTab(Context context, @DrawableRes int icon, CharSequence title) {
        this(context, null, icon, title,0,0);
    }

    public NavBarTab(Context context, @DrawableRes int icon, CharSequence title,int selectColor,int unselectColor) {
        this(context, null, icon, title,selectColor,unselectColor);
    }

    public NavBarTab(Context context, AttributeSet attrs, int icon, CharSequence title, int selectColor, int unselectColor) {
        this(context, attrs, 0, icon, title,selectColor,unselectColor);
    }

    public NavBarTab(Context context, AttributeSet attrs, int defStyleAttr, int icon, CharSequence title,int selectColor,int unselectColor) {
        super(context, attrs, defStyleAttr);
        if(selectColor != 0) {
            primary_color = selectColor;
        }
        if(unselectColor != 0) {
            unselect_color = unselectColor;
        }
    ...
    private void init(Context context, int icon, CharSequence title) {
        ...
        mIcon.setColorFilter(unselect_color);
        ...
        mTvTitle.setTextColor(unselect_color);
        ...
        mTvUnreadCount.setBackgroundResource(R.drawable.bubble);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            mIcon.setColorFilter(primary_color);
            mTvTitle.setTextColor(primary_color);
        } else {
            mIcon.setColorFilter(unselect_color);
            mTvTitle.setTextColor(unselect_color);
        }
